...
public interface SysUserMapper extends BaseMapper<SysUser> {
    ...
    @Select("SELECT u.*, r.role_name, d.dept_name, p.position_name " +
            ...
            "LEFT JOIN sys_department d ON u.dept_id = d.dept_id " +
            "LEFT JOIN sys_position p ON u.position_id = p.position_id " +
            "WHERE u.dept_id = #{deptId} AND p.level >= #{minLevel} AND u.deleted = 0 AND u.status = 1 " +
            "ORDER BY p.level ASC LIMIT 1")
    SysUser findApproverByDeptAndMinLevel(@Param("deptId") Long deptId, @Param("minLevel") int minLevel);
