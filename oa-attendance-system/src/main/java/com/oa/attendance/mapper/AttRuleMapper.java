...
public interface AttRuleMapper extends BaseMapper<AttRule> {
    ...
    @Select("<script>" +
            "SELECT * FROM att_rule " +
            "WHERE deleted = 0 AND enabled = 1 " +
            "AND (dept_id IN <foreach collection='deptIds' item='id' open='(' separator=',' close=')'>#{id}</foreach> OR dept_id IS NULL) " +
            "AND (effective_start_date IS NULL OR effective_start_date &lt;= #{attendanceDate}) " +
            "AND (effective_end_date IS NULL OR effective_end_date &gt;= #{attendanceDate}) " +
            "ORDER BY CASE WHEN dept_id IN <foreach collection='deptIds' item='id' open='(' separator=',' close=')'>#{id}</foreach> THEN 0 ELSE 1 END, create_time DESC " +
            "LIMIT 1" +
            "</script>")
    AttRule selectActiveRuleForDepts(@Param("deptIds") List<Long> deptIds, @Param("attendanceDate") LocalDate attendanceDate);
    ...
    @Select("<script>" +
            ...
            "LEFT JOIN sys_department d ON r.dept_id = d.dept_id " +
            "WHERE r.deleted = 0 AND r.enabled = 1 " +
            "AND (r.dept_id IN <foreach collection='deptIds' item='id' open='(' separator=',' close=')'>#{id}</foreach> OR r.dept_id IS NULL) " +
            "AND (r.effective_start_date IS NULL OR r.effective_start_date &lt;= #{attendanceDate}) " +
            "AND (r.effective_end_date IS NULL OR r.effective_end_date &gt;= #{attendanceDate}) " +
            "ORDER BY CASE WHEN r.dept_id IN <foreach collection='deptIds' item='id' open='(' separator=',' close=')'>#{id}</foreach> THEN 0 ELSE 1 END, r.work_start_time ASC, r.create_time DESC" +
            "</script>")
    List<AttRule> selectAvailableRulesForDepts(@Param("deptIds") List<Long> deptIds, @Param("attendanceDate") LocalDate attendanceDate);
}
