...
<template>
  <section class="document-page">
    ...
    <div v-if="documents.length" class="document-grid"><article v-for="item in documents" :key="item.docId" class="document-card" @click="openDetail(item)"><header><span class="document-symbol">文</span><div><h2>{{ item.title }}</h2><p>{{ item.authorName || '系统' }} · {{ formatTime(item.updateTime || item.createTime) }}</p></div></header><p class="document-preview">{{ preview(item.content) || '暂无内容摘要' }}</p><div v-if="splitTags(item.tags).length" class="document-tags"><span v-for="tag in splitTags(item.tags)" :key="tag">{{ tag }}</span></div><footer><button @click.stop="openDetail(item)">阅读</button><button v-if="can('document:update')" @click.stop="openEdit(item)">编辑</button><button v-if="can('document:delete')" class="danger" @click.stop="remove(item)">删除</button></footer></article></div>
    <div v-else-if="!loading" class="document-empty"><span>文</span><strong>{{ keyword ? '没有找到相关文档' : '暂无制度文档' }}</strong><p>{{ keyword ? '尝试更换关键词，或清空搜索条件。' : '管理员创建文档后会显示在这里。' }}</p></div>
