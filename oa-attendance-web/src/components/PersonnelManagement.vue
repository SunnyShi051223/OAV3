...
import { departmentApi, positionApi, roleApi, userApi } from '../api/oa'
...
const pageSize = computed(() => props.section === '员工管理' ? 10 : 6)
...
const positions = ref([])
const users = ref([])
const roles = ref([{ roleId: 1, roleName: '员工', roleCode: 'EMPLOYEE' }])
...
const assignableRoles = computed(() => roleCode.value === 'ADMIN' ? roles.value : roles.value.filter((item) => item.roleCode === 'EMPLOYEE'))
...
const currentRecords = computed(() => {
  if (props.section === '员工管理') return sortedEmployees.value
  if (props.section === '部门管理' && departmentView.value === 'tree') return treeRows.value
  return filteredRecords.value
})
const paginatedRecords = computed(() => currentRecords.value.slice((currentPage.value - 1) * pageSize.value, currentPage.value * pageSize.value))
const totalPages = computed(() => Math.max(1, Math.ceil(currentRecords.value.length / pageSize.value)))
...
async function loadReferenceData() {
  const roleRequest = hasPermission('role:query') ? roleApi.list().catch(() => ({ data: [] })) : Promise.resolve({ data: [] })
  const [departmentResult, positionResult, roleResult] = await Promise.all([departmentApi.list(), positionApi.list(), roleRequest])
  departments.value = departmentResult.data || []
  positions.value = positionResult.data || []
  if (roleResult.data?.length) roles.value = roleResult.data
}
...
function openCreate() {
  if (!can('add')) return
  const employeeRoleId = roles.value.find((item) => item.roleCode === 'EMPLOYEE')?.roleId || assignableRoles.value[0]?.roleId || 1
  const defaults = props.section === '员工管理'
    ? { employeeNo: '', username: '', password: '123456', realName: '', nickname: '', gender: '男', phone: '', email: '', birthDate: '', hireDate: '', deptId: roleCode.value === 'MANAGER' ? props.currentUser?.deptId : '', positionId: '', roleId: employeeRoleId, status: 1 }
...
watch([keyword, deptFilter, statusFilter, genderFilter, sort, departmentView], () => { currentPage.value = 1 })
watch(() => currentRecords.value.length, () => { if (currentPage.value > totalPages.value) currentPage.value = totalPages.value })
