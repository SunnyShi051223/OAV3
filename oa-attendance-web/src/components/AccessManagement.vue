...
const permAssignDialog = ref({ visible: false, role: null, permIds: [], error: '', saving: false })
...
async function load() {
  ...
  error.value = ''
  try {
    const [roleResult, menuResult] = await Promise.all([roleApi.list(), menuApi.list()])
    roles.value = roleResult.data || []
    menus.value = menuResult.data || []
    const permResult = await request.get('/roles/permissions')
    allPermissions.value = permResult.data || []
...
function openPermAssign(role) { permAssignDialog.value = { visible: true, role, permIds: [...(role.permissionIds || [])], error: '', saving: false } }
async function saveRolePerms() {
  const state = permAssignDialog.value
  state.saving = true
  state.error = ''
  try { await roleApi.assignPermissions(state.role.roleId, state.permIds.map(Number)); state.visible = false; showNotice('角色权限保存成功'); await load() } catch (err) { state.error = err.message } finally { state.saving = false }
}
...
<template>
  <section class="access-page">
    ...
    <div v-if="permAssignDialog.visible" class="dialog-mask" @click.self="permAssignDialog.visible = false"><form class="data-dialog menu-assign-dialog" @submit.prevent="saveRolePerms"><div class="dialog-heading"><div><h2>{{ permAssignDialog.role?.roleName }} · 权限授权</h2><p>勾选授予该角色的功能权限</p></div><button type="button" @click="permAssignDialog.visible = false">×</button></div><div class="perm-checkbox-grid"><label v-for="p in allPermissions" :key="p.permissionId" class="perm-item"><input v-model="permAssignDialog.permIds" type="checkbox" :value="p.permissionId" /><span><strong>{{ p.permissionName }}</strong><code>{{ p.permissionCode }}</code><small>{{ p.description || '' }}</small></span></label></div><div v-if="permAssignDialog.error" class="form-error">{{ permAssignDialog.error }}</div><div class="dialog-actions"><button type="button" @click="permAssignDialog.visible = false">取消</button><button class="primary-button" :disabled="permAssignDialog.saving">{{ permAssignDialog.saving ? '保存中…' : '保存权限' }}</button></div></form></div>
  </section>
</template>
...
.perm-checkbox-grid { max-height: 460px; overflow-y: auto; display: grid; gap: 6px; }
.perm-item { display: flex; align-items: flex-start; gap: 8px; padding: 8px 10px; border: 1px solid #e8ebf0; border-radius: 8px; }
.perm-item input { width: 15px; height: 15px; margin-top: 2px; flex-shrink: 0; }
.perm-item span strong { display: block; font-size: 13px; }
.perm-item span code { display: block; margin: 2px 0; font-size: 10px; }
.perm-item span small { display: block; color: #8f959e; font-size: 10px; }
